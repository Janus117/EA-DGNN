...
from utils.neighbor_loader import AllNeighborSampler
...
class CAWN(nn.Module):
    def __init__(
        ...
        neighbor_sampler: AllNeighborSampler,
        time_feat_dim: int,
    ...
    def compute_src_dst_node_temporal_embeddings(
        ...
        # count the appearances of nodes in the multi-hop graphs that are generated by random walks that
    ...
    def compute_node_temporal_embeddings(
        ...
        neighbor_time_features = self.time_encoder(
            torch.from_numpy(nodes_neighbor_delta_times)
            .float()
            .to(self.device)
            .flatten(start_dim=1)
        ).reshape(
        ...
        final_node_embeddings = self.walk_encoder(
            time_features=neighbor_time_features,
    ...
    def set_neighbor_sampler(self, neighbor_sampler: AllNeighborSampler):
        """
...
class PositionEncoder(nn.Module):
    ...
    def count_nodes_appearances(
        ...
        for idx, (src_node_id, node_interact_time) in enumerate(
            zip(src_node_ids, node_interact_times)
        ):
            ...
            for current_hop in range(self.walk_length + 1):
                for src_node_neighbor_id, src_node_neighbor_time in zip(
...
class WalkEncoder(nn.Module):
    def __init__(
        ...
        self.transformer_encoder = Attention(
            emb_size=self.attention_dim, heads=self.num_walk_heads, dropout=self.dropout
        )
    ...
    def forward(
        ...
        combined_features = torch.cat(
            [time_features, edge_features, neighbor_position_features], dim=-1
        )
