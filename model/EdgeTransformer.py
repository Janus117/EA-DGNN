...
class EdgeTransformer(torch.nn.Module):
    def __init__(
        ...
        self.src_attention = nn.ModuleList(
            [
                Attention(self.attention_emb_size, heads=2, dropout=dropout)
                for _ in range(2)
            ]
        )
        ...
        self.final_attention = nn.ModuleList(
            [
                Attention(self.final_state_size, heads=4, dropout=dropout)
                for _ in range(2)
            ]
        )
    ...
    def update_batch(self, src, dst, ts, msg, aux_msg=None):
        ...
        self.dst_history_timestamp[uni_dst, 1:] = self.dst_history_timestamp[
            uni_dst, 0:-1
        ]
    ...
    def dst_rep2(self, label_t):
        ...
        valid_dst_feature = self.dst_attention[0](
            q=valid_dst_feature,
            # key_padding_mask=self.dst_history_mask[valid_dst],
        )
        valid_dst_feature = self.dst_attention[1](
            q=valid_dst_feature,
            # key_padding_mask=self.dst_history_mask[valid_dst],
        )
    ...
    def src_rep2(self, label_srcs, label_t):
        ...
        valid_src_feature = self.src_attention[0](
            q=valid_src_feature,
            # key_padding_mask=self.src_history_mask[n_id - self.offset][
            #     valid_src
            # ].to(self.device),
        )
        valid_src_feature = self.src_attention[1](
            q=valid_src_feature,
            # key_padding_mask=self.src_history_mask[n_id - self.offset][
            #     valid_src
            # ].to(self.device),
    ...
    def forward(self, label_srcs, src_rep, dst_rep):
        ...
        final_rep = self.final_attention[0](q=final_rep)
        final_rep = self.final_attention[1](q=final_rep)
